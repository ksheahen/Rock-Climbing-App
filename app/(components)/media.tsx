...
function MediaComponent({
  ...
  const pickFromLibrary = async () => {
    ...
    if (!ok) return Alert.alert("Permission needed", "Enable photo library access.");
  ...
  const captureWithCamera = async () => {
    ...
    const next: MediaItem = { uri: a.uri, type: a.type === "video" ? "video" : "image" };
        ...
        <View style={styles.mediaBox}>
          ...
          >
            {items.length < maxItems && (
              <Pressable onPress={handleAdd} style={[styles.tile, styles.addTile]} hitSlop={8}>
            ...
            {items.map((m, idx) => (
              <View key={m.uri} style={styles.thumbWrap}>
                ...
                <View style={styles.badge}>
                  <Text style={styles.badgeText}>{m.type === "image" ? "IMG" : "VID"}</Text>
                </View>

                <Pressable onPress={() => removeAt(idx)} style={styles.removeBtn} hitSlop={8}>
