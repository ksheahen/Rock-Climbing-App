import { Challenge, ChallengeInsert, ChallengeUpdate } from '../types/Challenge.ts';
...
export const createChallenge = async (newChallenge: ChallengeInsert): Promise<Challenge | null> => {
  const { data, error } = await table('challenge')
    .insert(newChallenge)
    .select()
    .maybeSingle();
