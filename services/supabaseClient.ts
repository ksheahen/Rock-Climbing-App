...
if (process.env.NODE_ENV === 'test') {
  ...
  if (!SUPABASE_KEY) throw new Error('SUPABASE_SERVICE_ROLE_KEY is missing in .env');
} else {
  // Use anon/publishable key for normal operations
  SUPABASE_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;
  if (!SUPABASE_KEY) throw new Error('NEXT_PUBLIC_SUPABASE_ANON_KEY is missing in .env');
}
